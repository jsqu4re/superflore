...
def get_pr_text(delta=None, missing_deps=None, comment=None, markup='```'):
    msg = ''
    if comment:
        msg += comment + '\n'
...
def save_pr(overlay, delta, missing_deps, comment, title=''):
    with open('.pr-title.tmp', 'w') as title_file:
        title_file.write(title)
    with open('.pr-message.tmp', 'w') as pr_msg_file:
        pr_msg_file.write(get_pr_text(delta, missing_deps, comment))
...
def file_pr(overlay, delta, missing_deps, comment, distro=None, title=''):
    try:
        msg = get_pr_text(delta, missing_deps, comment)
        overlay.pull_request(msg, distro, title=title)
